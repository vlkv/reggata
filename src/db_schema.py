...
class HistoryRec(Base):
    ...
    #TODO При работе с одним и тем же хранилищем под разными ОС (если хранилище на флешке, например)
    #возникает ошибка: слеши os.sep разные и сохраняются в HistoryRec и DataRef в том виде, как принято в конкретной ОС.
    #Поэтому нужно сохранять в БД только прямые слеши. А после извлечения данных из БД, менять слеши на обратные, если ОС Windows.
